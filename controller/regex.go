	...
	for len(firstPacket) < regexpProbeLimit {
		...
		if n > 0 {
			...
			if len(matched) > 0 {
				...
				handleRegexpMatch(ctx, conn, rule, matched, firstPacket)
				return
			}
		}
...
func handleRegexpMatch(ctx context.Context, conn net.Conn, rule *config.Rule, matched []*config.Target, firstPacket []byte) {

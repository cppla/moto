...
当同一项服务同时拥有多个入口、出口、隧道或跨地域节点时，静态优先级很快会过时，普通轮询又无法区分线路质量。Moto 把故障切换、首包分类、EWMA 延迟学习、Top-2 线路竞速、熔断与半开恢复组合在一个进程里；应用只连接一个稳定地址，线路选择和故障绕行由 Moto 完成。

Moto 工作在 TCP 字节流层，不终止 TLS、不改写协议，也不要求业务接入 SDK。HTTP、HTTPS、WebSocket、SSH、SOCKS5 以及私有 TCP 协议都可以从同一个透明转发核心受益。
...
Moto 把复杂性留在网关内部，把简单留给应用：客户端只管连接一个稳定地址，Moto 负责在不断变化的网络里找到更值得走的路。
